...
X_train = load_df(conn, "X_train_auto_base", exclude_cols=["id"])
X_val = load_df(conn, "X_val_auto_base", exclude_cols=["id"])
y_train = load_df(conn, "y_train_auto", exclude_cols=["id"])["call_counts"].to_numpy()
y_val = load_df(conn, "y_val_auto", exclude_cols=["id"])["call_counts"].to_numpy()
...
print("=" * 80)
pois_result = sm_poisson_glm(X_train_const, y_train, model_output_dir=auto_model_dir)
...
print("=" * 80)
zip_result = sm_zip(X_train_const, y_train, model_output_dir=auto_model_dir, start_params=zip_start_params)
...
print("=" * 80)
zinb_result = sm_zinb(X_train_const, y_train, model_output_dir=auto_model_dir, start_params=zinb_start_params)
